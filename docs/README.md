...
- [`../.env.example`](../.env.example) - copy this to `.env` and fill in your
  local profile.
- [`../CHANGELOG.md`](../CHANGELOG.md) - notable changes.

The macOS launchd jobs are generated from the `SCHED_*` schedule in `.env` by
`cadence schedule apply` — see [Configuration](CONFIGURATION.md#schedule).
