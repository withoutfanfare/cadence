...
Project-local `cadence/.env` is supported for manual commands now. The launchd
jobs generated by `cadence schedule apply` do not yet set `CADENCE_CONFIG`, pass
`--config`, or set `WorkingDirectory`, so they do not safely run against a
project-local config after launchd starts them.

Use scheduled runs only with the existing root `.env` compatibility path, or
after adding explicit launchd config support. To generate and load the current
jobs:
...
default cadence is hourly, staggered 15 minutes apart. With the root `.env`
compatibility path, change it any time by setting `SCHED_<STAGE>` and re-running
`cadence schedule apply` — see [Configuration](CONFIGURATION.md#schedule) for the
format. (The advance and conduct jobs are added later by `cadence autonomous on`; see
