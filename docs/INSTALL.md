...
Cadence will find this file when you run commands from `/path/to/app`. Existing
root `.env` installs still work, but new project profiles should use
`cadence/.env`.
...
Cadence generates and loads the launchd jobs for you from the schedule in the
active config file:
...
`SCHED_<STAGE>` in the active config file and re-running `cadence schedule apply` — see
...
smaller environment than your terminal, so use `RUNNER_PATH_PREPEND` in the
active config file if project tools need a custom path.
